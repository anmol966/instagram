...
router.post("/signup", async (req, res) => {
  try {
    ...
    if (password.length < 4) {
      return res.status(400).json("Password must be up to 4 characters.");
    }
    ...
    const token = genToken(newUser.rows[0].user_id, newUser.rows[0].email);
...
router.post("/login", async (req, res) => {
  try {
    ...
    const token = genToken(user.rows[0].user_id, user.rows[0].email);
...
router.get("/users/:id", auth, async (req, res) => {
  try {
    ...
    const isAdmin = admin_user_ids.includes(email);
...
// Delete a user
router.delete("/users/:delete_user_id", auth, async (req, res) => {
  // get the user id from params
  try {
    const { user_id, email } = req.user;
    const { delete_user_id } = req.params;
    ...
    if (admin_user_ids.includes(delete_user.rows[0].email)) {
      return res.status(404).json({ message: "You can't delete an admin" });
    }
    ...
    return res.status(404).json("You need admin permission to delete a user");
  } catch (err) {
